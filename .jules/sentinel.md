...
## 2026-05-25 - API Key Leakage via Base64-Encoded Error Messages
**Vulnerability:** The application correctly sanitized raw and URL-encoded API keys from error messages, but failed to account for scenarios where external APIs echo the credential in a base64-encoded format (e.g., in a `Bearer` token context). This bypassed the redaction logic, exposing the encoded API key in logs.
**Learning:** Credentials can be transformed into base64 format during HTTP transit. If the error response reflects this transformed state, it will bypass raw string matching.
**Prevention:** When implementing dynamic secret redaction (`_sanitize_error_msg`), compute the base64-encoded representation of the credential and redact the encoded variation in addition to the raw and URL-encoded secret.
