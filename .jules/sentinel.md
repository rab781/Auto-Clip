...
## 2026-02-13 - Prevent DoS and Information Disclosure via Subprocess Error Truncation
**Vulnerability:** When running subprocesses (like FFmpeg) and catching errors, logging or raising the full `result.stderr` (or capturing it from the beginning using `[:200]`) can lead to Denial of Service (DoS) via log flooding if the external tool returns a massive error trace. Furthermore, since FFmpeg prints its actual error message at the very end of its output trace, slicing from the beginning (`[:200]`) often misses the actual error and captures irrelevant build information.
**Learning:** External tool error streams must be treated as untrusted and potentially massive. Slicing from the beginning often misses the crucial error context, while capturing the whole payload is dangerous. Additionally, defensively handling cases where `stderr` might be `None` prevents runtime `TypeError` crashes.
**Prevention:** Always truncate `subprocess.run` error outputs from the *end* (e.g., `result.stderr[-500:]`) to capture the actual error context safely, and use a defensive conditional (e.g., `result.stderr[-500:] if result.stderr else ''`) to prevent crashes if standard error is empty.
