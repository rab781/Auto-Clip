...
## 2026-06-16 - API Key Leakage via Case-Insensitive URL-Encoded Error Messages
**Vulnerability:** The application was using case-sensitive string matching (`msg.replace()`) to sanitize URL-encoded API keys from error messages. Since URL-encoding generates hexadecimal digits which can be represented in either upper or lower case (e.g., `%3A` vs `%3a`), error messages reflecting the API key with a different casing than the internally encoded string would bypass redaction and leak credentials.
**Learning:** Security validations and data sanitization routines dealing with encodings (like URL encoding or hex data) must account for casing variations. External services often modify or normalize casing when reflecting data.
**Prevention:** Use regular expressions with case-insensitive flags (e.g., `re.sub(..., flags=re.IGNORECASE)`) when redacting dynamically encoded strings to ensure all casing permutations are securely sanitized.
