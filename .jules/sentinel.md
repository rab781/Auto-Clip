...
## 2026-06-25 - API Key Leakage via Base64-Encoded Error Messages
**Vulnerability:** The application was passing external API error responses (`response.text`) directly into printed logs and raised exceptions. The application correctly sanitized raw API keys (`CHUTES_API_KEY`) and URL-encoded variations from these error messages. However, it failed to redact Base64-encoded variations of the API key, which could be reflected back by the external API in certain HTTP errors.
**Learning:** External APIs can reflect credentials in various encodings. Redacting only the plaintext and URL-encoded versions of a credential is an incomplete defense. Base64 is a common encoding format (e.g., used in Basic Auth or token handling) and must be considered.
**Prevention:** When implementing dynamic secret redaction (e.g., `_sanitize_error_msg`), proactively encode the credential into common formats, including Base64 (`base64.b64encode`), and redact those variations from any untrusted output strings in addition to the plaintext secret.
