...
## 2024-05-27 - Prevent DoS via Improper FFmpeg Log Slicing
**Vulnerability:** When processing FFmpeg errors, exception handlers were either logging the entire `result.stderr` output or slicing it from the beginning (`[:200]`). Because FFmpeg prints its critical error reasons at the very end of its often massive stderr output (which contains banner info and progressive status updates), early truncation loses the actual error data, and full output logging creates a Denial of Service (DoS) vulnerability via log/memory exhaustion.
**Learning:** For continuous CLI tools like FFmpeg, the beginning of standard error is useless banner data; the end contains the fatal error. Capturing unbounded stderr directly exposes the application to resource exhaustion if an attacker supplies a file designed to fail verbosely.
**Prevention:** Always truncate CLI tool error outputs from the end of the string (e.g., `result.stderr[-500:]`) and handle potential `None` returns safely (e.g., `if result.stderr else ''`) to ensure critical observability is maintained while protecting application memory.
