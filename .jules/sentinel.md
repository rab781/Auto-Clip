...
**Prevention:** Always string-slice and truncate raw error messages (e.g., `str(e)[:500]`) before raising them internally or logging them.## 2026-04-15 - Prevent DoS via Log Flooding and Information Exposure
**Vulnerability:** Exception payloads returned by external tools like `ffmpeg` or API calls were thrown without length constraints. Attackers could manipulate input to cause these dependencies to fail and return massive error payloads resulting in log flooding, resource exhaustion, or accidental exposure of internal tool state.
**Learning:** External error payloads are untrusted inputs. Taking large error responses verbatim exposes an application to Information Exposure and Denial of Service (DoS) attacks via memory or log exhaustion.
**Prevention:** Always string-slice and truncate raw error messages. For `ffmpeg`, always slice from the end (e.g., `result.stderr[-500:] if result.stderr else ''`) because the actual error message is located at the very end of the output trace.
