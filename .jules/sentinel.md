...
## 2026-02-12 - API Key Leakage via Case-Sensitive URL Encoding Redaction
**Vulnerability:** The error message sanitization logic (`_sanitize_error_msg`) attempted to redact URL-encoded API keys by matching the exact output of `urllib.parse.quote`. However, this left the application vulnerable to credential leakage because external APIs or web frameworks often reflect URL-encoded characters in lowercase hex (e.g., `%3a`), which did not match the strict uppercase hex (e.g., `%3A`) produced by python's `quote` function.
**Learning:** URL-encoding redaction must be case-insensitive because the hex digits in percent-encoding (`%A-F` vs `%a-f`) are equivalent and unpredictable in external error responses.
**Prevention:** Use regular expressions with the case-insensitive flag (`re.IGNORECASE`) when sanitizing URL-encoded secrets from external responses, rather than relying on strict string replacement against a single standard encoding format.
