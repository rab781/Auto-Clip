...
## 2026-03-25 - Prevent DoS and Information Exposure via Massive FFmpeg Error Logs
...
## 2026-06-08 - API Key Leakage via Base64-Encoded Error Messages
**Vulnerability:** The application previously sanitized raw API keys (`CHUTES_API_KEY`) and URL-encoded variations from error messages. However, it failed to account for scenarios where external APIs might echo the credential in a base64-encoded format within their error payload (e.g., during failed authorization parsing or header validation). This gap bypassed the existing redaction logic, potentially exposing the base64-encoded API key in local logs.
**Learning:** Security measures like secret redaction must cover all common encodings that might be reflected by external systems. Credentials can be transformed (specifically, URL-encoded or base64 encoded) during transit. If an error response reflects this transformed state, it will bypass raw string matching.
**Prevention:** When implementing dynamic secret redaction (`_sanitize_error_msg`), always consider and test for common encodings. Specifically, encode the credential into base64 and redact that variation in addition to the raw secret and the URL-encoded version.
