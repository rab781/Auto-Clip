...
## 2026-03-24 - DoS via Log Flooding from FFmpeg Stderr Output
**Vulnerability:** The application was capturing the entire `stderr` output from FFmpeg subprocess calls and including it in raised exceptions. FFmpeg can produce massive amounts of output on `stderr`, especially during long processes or if provided with crafted inputs designed to generate warnings. If an attacker caused a failure, the application would attempt to log or bubble up the entire massive string, leading to Denial of Service (DoS) via log flooding and potential memory exhaustion.
**Learning:** External tool outputs (like `stdout` and `stderr`) are untrusted and can be arbitrarily large. Additionally, FFmpeg typically places the actual critical error message at the very end of its output stream. Therefore, truncating from the beginning (e.g., `result.stderr[:200]`) will miss the actual error reason and just capture generic header logs.
**Prevention:** Always truncate large CLI tool outputs by slicing from the end (e.g., `result.stderr[-500:]`) to capture the actual error context while strictly bounding the maximum string size. Furthermore, handle `None` cases safely (e.g., `result.stderr[-500:] if result.stderr else ''`) to prevent `TypeError`s during the slice operation.