...
## 2026-06-25 - API Key Leakage via Case-Insensitive URL-Encoded Error Messages
**Vulnerability:** The application was redacting URL-encoded API keys from error messages using a case-sensitive string replacement (`msg.replace`). Because URL encoding uses hexadecimal characters, the casing can vary depending on the upstream service or library (e.g., `%2f` vs `%2F`). A case-sensitive replacement could fail to redact a valid encoded secret if the casing differs from the application's internal URL encoding logic, leading to credential leakage.
**Learning:** When sanitizing external inputs or error responses for transformed secrets (like URL-encoded credentials), string matching must account for valid format variations, particularly hexadecimal character casing.
**Prevention:** Use regular expressions with the case-insensitive flag (`re.sub` + `re.IGNORECASE`) to perform redactions of URL-encoded secrets instead of basic string replacement.
