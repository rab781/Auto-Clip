...
## 2026-03-24 - DoS and TypeError via Improper Subprocess Error Truncation
**Vulnerability:** When logging FFmpeg errors (`result.stderr`), the code either logged the unbounded output or truncated it from the beginning (`[:200]`). Massive logs cause DoS via log flooding. Truncating from the beginning misses the actual error details printed at the end by FFmpeg. Furthermore, blindly slicing `result.stderr` can cause a `TypeError` if `stderr` is `None`.
**Learning:** Capturing entire payloads from external subprocesses can cause DoS. However, when slicing to prevent this, you must slice from the correct end depending on the tool's behavior (FFmpeg logs errors at the end) and handle potential `None` types safely.
**Prevention:** Always slice FFmpeg's `result.stderr` from the end using a conditional expression (e.g., `result.stderr[-500:] if result.stderr else ''`) before raising exceptions to prevent DoS, ensure error visibility, and avoid crashes.
