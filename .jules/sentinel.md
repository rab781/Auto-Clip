...
## 2026-05-25 - Prevent API Key Leakage via Base64 Encoding
**Vulnerability:** The application was catching API HTTP errors and formatting `response.text` directly into its own exception messages. The redaction logic properly handled plaintext and URL-encoded versions of the `CHUTES_API_KEY`, but missed base64-encoded versions. If an external API logged an error reflecting a base64-encoded key, the `_sanitize_error_msg` function would fail to catch it, leaking the sensitive credential into logs.
**Learning:** Security controls designed to redact secrets from untrusted input must account for all common encoding mechanisms. A credential might be echoed back transformed into a base64 string during transit or API processing.
**Prevention:** Extend dynamic secret redaction capabilities to include base64-encoded forms of the API key using `base64.b64encode().decode('utf-8')` to ensure comprehensive protection against encoded credential leakage in error messages.
