...
This guide walks you through configuring Auto-Clip Bot V2 settings, such as styling your subtitles and limiting download sizes, to customize the generated clips for your brand.

## Step 1: Open the Configuration File

Configure the bot by editing the `content-bot/config.py` file. Open this file in your text editor.

## Step 2: Configure Caption Settings

To change the styling of your subtitles, modify the `CAPTION_SETTINGS` dictionary.

Select between `animated` or `simple` styles, change the `font`, or update the `highlight_color`.
...
To adjust the size of the generated vertical clips, modify the `VIDEO_SETTINGS` dictionary.
...
Verify your configuration by performing a dry run before processing long videos. Run the following command:
