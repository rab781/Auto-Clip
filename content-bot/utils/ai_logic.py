...
def _parse_clips_json(content: str) -> list:
    ...
    # ⚡ Bolt Optimization: Use string search instead of greedy regex for large text blocks
    # Impact: Replaces catastrophic O(N^2) backtracking (due to `[\s\S]*`) with native C-level `find()`/`rfind()` (O(N)).
    # Measurement: Compare parsing time of large LLM outputs using time.time().
