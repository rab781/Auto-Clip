...
def transcribe_audio(audio_path: str, max_retries: int = 3, chunk_duration: int = 300) -> dict:
    ...
    for idx, start_ts, result in results:
        ...
        # ⚡ Bolt Optimization: Use list accumulation instead of += for O(N) string building
        # Impact: Prevents O(N^2) memory reallocation overhead when building large transcripts
        # Measurement: Compare memory usage and speed when merging many chunks
