...
def _parse_clips_json(content: str) -> list:
    ...
    # ⚡ Bolt Optimization: Use native string methods instead of greedy regex
    # Impact: Significantly speeds up parsing of large LLM outputs by preventing
    # catastrophic backtracking (O(N^2) complexity) inherent in greedy regex matches.
    # Measurement: Benchmark regex vs native find/rfind on large text inputs.
