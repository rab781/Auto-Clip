...
def create_final_clip(
    ...
    # ⚡ Bolt Optimization: Use in-memory clip duration to calculate thumbnail timestamp
    # Impact: Eliminates a synchronous blocking `ffprobe` subprocess call per clip, saving significant IO overhead.
    # Measurement: Compare pipeline generation time for multiple clips with and without this change.
