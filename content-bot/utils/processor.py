...
def create_final_clip(
    ...
    # ⚡ Bolt Optimization: Calculate thumbnail timestamp using in-memory clip_info
    # Impact: Eliminates blocking `ffprobe` subprocess call per clip in generate_thumbnail
    # Measurement: Timing `generate_thumbnail` with and without passing the timestamp
    clip_duration = clip_info.get("end", 30.0) - clip_info.get("start", 0.0)
    thumbnail_timestamp = max(0.0, clip_duration / 3.0)
