...
def _get_audio_mix_filter(bgm_volume: float = None) -> str:
    ...
    # ⚡ Bolt Optimization: Removed size parameter from aloop and duration calculation
    # Impact: Avoids unnecessary synchronous subprocess calls (like ffprobe for video duration) when constructing FFmpeg filter graphs.
    # Features like amix=duration=first combined with aloop=loop=-1 natively handle truncation.
