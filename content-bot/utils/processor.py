...
def _get_smart_crop_x(video_path: str, width: int, height: int) -> str:
    """Helper to determine crop X position using Smart Crop or Center Crop"""
    # Default: Center Crop
    crop_x = "(in_w-out_w)/2"
...
def _get_audio_mix_filter(duration: float, bgm_volume: float = None) -> str:
    """Helper to generate audio mixing filter string"""
...
def burn_captions(video_path: str, srt_path: str, output_path: str) -> str:
    ...
    if result.returncode != 0:
        # Fallback: simple subtitle format if complex one fails
        print("! Trying simpler subtitle format...")
        # Recalculate simple filter
        srt_escaped = str(srt_path).replace("\\", "/").replace(":", "\\:").replace("'", r"'\''")
        cmd = [
            "ffmpeg", "-y",
            "-i", str(video_path),
...
def _create_final_clip_optimized(
    ...
    """
    Optimized pipeline: Convert to vertical + Burn captions + Add BGM in ONE pass.
    """
    output_path = Path(output_path)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    # Video Scale & Crop
    # [0:v] -> [v_cropped]
    filters.append(f"[0:v]scale=-1:{height},crop={width}:{height}:{crop_x}:0[v_cropped]")

    current_video_stream = "[v_cropped]"

    # Subtitles
    if subtitle_path and Path(subtitle_path).exists():
        sub_filter = _get_subtitle_filter(subtitle_path)
        # [v_cropped] -> [v_subbed]
        filters.append(f"{current_video_stream}{sub_filter}[v_subbed]")
        current_video_stream = "[v_subbed]"
    ...
    cmd.extend([
        "-filter_complex", filter_complex,
        "-map", current_video_stream,
        "-map", current_audio_stream,
        "-c:v", "libx264",
        "-crf", "18",       # High quality (visually lossless)
        "-preset", "slow",   # Better compression efficiency
        "-pix_fmt", "yuv420p",
...
def create_final_clip(
    ...
    Attempts optimized single-pass first, falls back to sequential if needed.
    """
    ...
    # Prepare dependencies (SRT and BGM)
