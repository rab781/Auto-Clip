...
def _get_audio_mix_filter(bgm_volume: float = None) -> str:
    ...
    # ⚡ Bolt Optimization: Removed size= calculation from aloop filter and _get_video_duration dependency
    # Impact: Avoids unnecessary synchronous `ffprobe` process creation per video just to get duration.
    # The `amix=duration=first` parameter naturally limits the infinite loop to the video duration.
    return (
