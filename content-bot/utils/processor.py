...
def _get_smart_crop_x(video_path: str) -> str:
    """Helper to determine crop_x expression for ffmpeg"""
    # Default: Center Crop
    crop_x = "(in_w-out_w)/2"
...
def _get_audio_mix_filter(duration: float, bgm_volume: float, original_volume: float) -> str:
    ...
    # Note: Inputs must be mapped by the caller
    # Using [1:a] for BGM and [0:a] for original is standard for 2 inputs
    return (
...
def _create_final_clip_optimized(
    ...
    """
    Optimized pipeline that combines scaling, cropping, subtitles, and BGM
    ...
    crop_x = _get_smart_crop_x(video_path)
    # Start chain with [0:v] implicitly or explicitly?
    # Usually implicit for first filter if input 0 is video.
    # But better to be explicit in complex filter.
    video_filter_chain = f"scale=-1:{height},crop={width}:{height}:{crop_x}:0"

    if srt_path:
        subtitle_filter = _get_subtitle_filter(srt_path)
        # Combine: crop then subtitle
    ...
    filter_complex = f"[0:v]{video_filter_chain}[outv]"
    ...
    if bgm_path:
        ...
        # _get_audio_mix_filter uses [1:a] and [0:a] hardcoded, which matches our inputs
...
def create_final_clip(
    ...
    # Step 1: Generate Captions (SRT or ASS)
    ...
    # Step 3: Run Optimized Pipeline (One-pass FFmpeg)
    final_video_path = output_dir / f"{base_name}.mp4"

    # Validate subtitle path
    srt_arg = str(subtitle_path) if (subtitle_path and subtitle_path.exists()) else None
    ...
    try:
        ...
        if srt_arg:
            ...
            captioned_video = burn_captions(vertical_video, srt_arg, str(captioned_path))
        else:
